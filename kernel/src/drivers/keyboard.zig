...
fn keyboard_callback(r: cpu.int.Registers) callconv(.c) void {
    const scancode = cpu.ports.byte_in(0x60);
    if (scancode > SC_MAX) {
...
pub fn init() void {
    cpu.int.register_interrupt_handler(cpu.int.IRQ1, &keyboard_callback);
}
